...
DLA_MODELS=() #('vitb_imagenet_doclaynet_tecaher') # CNN ViT-b simkd_ViT-t reviewkd_ViT-t simkd_CNN reviewkd_CNN
for model in ${MODELS[@]}; do
    for dataset in ${DATASETS[@]}; do
        ...
        for dla_model in  ${DLA_MODELS[@]}; do
            ...
            nohup bash script/llama_eval.sh 2 $model $dataset space $dla_model  > space.out &
